...
public final class UnsetSpawnCommand implements BasicCommand {

    private final HySpawn plugin;

    public UnsetSpawnCommand(HySpawn plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(@NotNull CommandSourceStack stack, @NotNull String[] args) {
        ...
        if (!plugin.getSpawnDataManager().hasGlobalSpawn()) {
            plugin.getMessageUtil().sendMessage(player, "spawn-not-set");
            return;
        }

        plugin.getSpawnDataManager().removeGlobalSpawn();
        plugin.getMessageUtil().sendMessage(player, "spawn-unset");
    }
